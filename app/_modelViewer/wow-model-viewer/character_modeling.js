...
function optionsFromModel(model, fullOptions) {
  ...
  // special case for pandaren
  const raceToModelId = race === 26 ? (gender === 0 ? 47 : 48) : race * 2 - (gender === 0 ? 1 : 0);
  return {
 ...
 */
async function getDisplaySlot(item, slot, displayId, contentPath) {
  ...
  try {
    await fetch(`${contentPath}meta/armor/${slot}/${displayId}.json`, {
      cache: "force-cache",
...
async function findItemsInEquipments(equipments) {
  for (const equipment of equipments) {
    ...
    const displayedItem =
      Object.keys(equipment.transmog).length !== 0 ? equipment.transmog : equipment.item;
 ...
 */
async function findRaceGenderOptions(race, gender, contentPath) {
  // special case for pandaren
  const customizationId =
    race === 26 ? (gender === 0 ? 47 : 48) : race * 2 - (gender === 0 ? 1 : 0);
  const options = await fetch(`${contentPath}meta/charactercustomization/${customizationId}.json`, {
    cache: "force-cache",
  }).then((response) => response.json());
  if (options.data) {
    return options.data;
